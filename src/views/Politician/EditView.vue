			...
			<form @submit.prevent="updatePerson" class="bg-white p-4 shadow rounded-lg w-4/5">
				<div class="divide-y divide-gray-100 mx-8">
					<div class="px-4 py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0 bg-gray-100">
						...
						<input id="name" v-model="person.name" type="text"
					...
					<div class="px-4 py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
						...
						<input id="gender" v-model="person.gender" type="text"
							class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0 border rounded px-2 py-1 w-full" />
					</div>
					<div class="px-4 py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0 bg-gray-100">
						<label for="about" class="text-sm font-medium leading-6 text-gray-900 px-4">Details</label>
						<textarea id="about" v-model="person.about"
							class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0 border rounded px-2 py-1 w-full"
							rows="3"></textarea>
					</div>
						...
						<div class="mt-4 flex justify-start">
							<div>
								...
								<p class="mt-2 flex items-center text-lg text-gray-500">
									Synced
									<CheckCircleIcon class="flex-shrink-0 mr-1.5 h-5 w-5 text-green-400 mx-1" aria-hidden="true" />
								</p>
								</div>
								<div v-else>
								<p class="mt-2 flex items-center text-lg text-gray-500">
									Not Synced
									<XCircleIcon class="flex-shrink-0 mr-1.5 h-5 w-5 text-red-400 mx-1" aria-hidden="true" />
								</p>
								</div>
							</div>
						</div>
						<div class="flex flex-row gap-8 shrink-0 mt-2">
							<button :disabled="image.detected" @click.prevent="syncImageToPerson(image, person)"
								class="bg-cyan-500 shadow-lg shadow-cyan-500/50 text-white font-bold py-2 px-4 rounded">
								Connect
							</button>
							<button class="bg-red-500 shadow-lg shadow-red-500/50 text-white font-bold py-2 px-4 rounded">
								Delete
							</button>
						</div>
					</div>
