...
<template>
	<div class="grid grid-col grid-cols-2 mt-10 gap-2 w-9/12 mx-auto">
		<div class="rounded-lg p-4 shadow-md bg-gray-100">
			<div class="flex items-center justify-center w-full my-2 mb-4">
				<label for="dropzone-file"
					...
					@drop="facePlusStore.dropHandler" @dragover.prevent="">
					<div class="flex flex-col items-center justify-center pt-5 pb-6">
						<svg aria-hidden="true" class="w-10 h-10 mb-3 text-sky-500" fill="none" stroke="currentColor"
					...
					<input id="dropzone-file" type="file" class="hidden" @change="facePlusStore.uploadHandler"
						accept="image/png, image/jpeg" />
				</label>
			</div>
			<hr />
			<figure class="max-w-lg mt-4 flex mx-auto position: relative">
				<img id="canva1" class="h-auto rounded-lg shadow-lg shadow-gray-800 hover:shadow-sky-200"
					:src="facePlusStore.uploadedInfo.imgurl" alt="user provided image" />
			...
			<div class="flex justify-center">
				<button class="flex p-2 mt-8 pr-4 rounded-md bg-sky-400 text-slate-200 hover:bg-sky-300 shadow-lg"
					...
					:disabled="facePlusStore.status.loading" @click.prevent="facePlusStore.detectFaces">
					...
					<div v-else role="status" class="flex justify-center">
						<svg class="inline mr-2 w-6 h-6 text-gray-200 animate-spin fill-pink-600" viewBox="0 0 100 101"
		...
		<div class="rounded px-2 border-1 shadow-md bg-gray-100">
			...
			<hr class="mt-2" />
			<div v-for="face in facePlusData.faces" class="flex mt-2 p-2 bg-white shadow rounded-lg items-center">
				<img class="rounded-full m-2 h-10 w-10" src="https://randomuser.me/api/portraits/thumb/men/75.jpg"
					alt="random user" />
				<div class="ml-2 m-2">
					<p class="font-medium">{{ face.face_token }}</p>
					<p class="font-light text-slate-400">{{ face.face_token }}</p>
				</div>
			</div>
		</div>
	</div>
